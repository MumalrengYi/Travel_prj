...
public class BoardController {
	...
	public String main(@RequestParam("board_info_idx") int board_info_idx,
						@RequestParam(value="page", defaultValue="1") int page, //페이징처리를 위해 현재 페이지를 받아오기
						Model model) {
		model.addAttribute("board_info_idx", board_info_idx); //글쓰기 버튼 링크로 달아줘야해서 게시판종류(idx) model로 넘기기
	...
	public String read(@RequestParam("board_info_idx")int board_info_idx,
						...
						Model model) {
		model.addAttribute("board_info_idx", board_info_idx); //목록보기 떄문에 담는거
		model.addAttribute("content_idx", content_idx); //수정or삭제하기 눌렀을 때 어떤거를 작동시킬지 알아야하기 떄문에 index번호가 필요하다.
		
		ContentBean readContentBean = boardService.getContentInfo(content_idx);
		model.addAttribute("readContentBean",readContentBean);
		
		model.addAttribute("loginUserBean", loginUserBean); //글쓴이와 로그인한이와 같은 계정인이 확인하는 과정을 위해 loginUserBean을 가져와 모델에 주입
		model.addAttribute("page", page);
		return "board/read";
	}
	@GetMapping("write")
